...
CommonNeighborsLP::CommonNeighborsLP( const Network& network ) : LinkPredictor(network) {
}

CommonNeighborsLP::~CommonNeighborsLP() {
}

double CommonNeighborsLP::generateScore( index_v indexVertex1, index_v indexVertex2 ) {
    return network.intersect(network.getNeighbors(indexVertex1), network.getNeighbors(indexVertex2)).size();
}
