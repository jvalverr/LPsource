...
ResourceAllocationLP::ResourceAllocationLP( const Network& network ) : LinkPredictor(network) {
}

ResourceAllocationLP::~ResourceAllocationLP() {
}

double ResourceAllocationLP::generateScore( index_v indexVertex1, index_v indexVertex2 ){
    double result = 0.0;
	...
	for( index_v i = 0; i < commonNeighbors.size(); i++) {
	    double degree = this->network.getDegree( commonNeighbors[i] );
