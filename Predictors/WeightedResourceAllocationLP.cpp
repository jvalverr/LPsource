...
WeightedResourceAllocationLP::WeightedResourceAllocationLP( const Network& network ) : LinkPredictor(network) {
     if(this->network.isNetworkWeighted() == false)
        cerr << "  Network is not weighted. We will consider 1 as the weight of all links\n";
}

WeightedResourceAllocationLP::~WeightedResourceAllocationLP() {
}

double WeightedResourceAllocationLP::generateScore( index_v indexVertex1, index_v indexVertex2 ){
    double result = 0.0;
	...
	for( index_v i = 0; i < commonNeighbors.size(); i++) {
        double den = this->network.getStrengthOfNode( commonNeighbors[i] );
        if(den != 0.0 ){
